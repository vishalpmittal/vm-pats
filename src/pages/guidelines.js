...
export async function renderGuidelines(container, slug) {
    container.innerHTML = "";
    try {
        const resp = await fetch(`/api/guidelines/${slug}`);
        if (!resp.ok) {
            container.appendChild(el("div", { className: "page-header" }, el("h1", {}, "Guidelines")));
            container.appendChild(el("div", { className: "empty-state glass-card" }, el("p", {}, "Could not load guidelines.")));
            return;
        }
        const data = await resp.json();
        const header = el("div", { className: "page-header" }, el("h1", {}, data.title));
        ...
        deleteBtn.addEventListener("click", async () => {
            if (!confirm(`Delete guideline "${data.title}"?`))
                return;
            try {
