...
type SortKey = "rank" | "company" | "sector" | "type" | "roles";
type SortDir = "asc" | "desc";

function showCompanyModal(opts: { title: string; initial?: Company; onSave: (data: { company: string; sector: string; type: string; careersUrl: string }) => Promise<boolean> }): void {
...
export async function renderCompanies(container: HTMLElement): Promise<void> {
  ...
  function renderTable() {
    ...
    for (const c of companies) {
      ...
      const companyCell = el("span", { className: "company-name-cell" }, companyLink, editBtn);
